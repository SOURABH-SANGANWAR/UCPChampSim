...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t** umons_counts;
    uint16_t * partitions;
    cell*** umon;
    uint64_t test_sets = 32;    // or LLC_SET/sampling;
    uint32_t LATENCY;
    BLOCK **block;
    int fill_level;
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
        ...
        LATENCY = 0;
        if(v1=="LLC"){
            uint16_t temp = NUM_CPUS - (NUM_WAY%NUM_CPUS);
            partitions = (uint16_t*)malloc(NUM_CPUS*(sizeof(uint16_t)));
            for (uint32_t i=0; i<NUM_CPUS; i++) {
                if(i<temp){
                    partitions[i]=NUM_WAY/NUM_CPUS;
                }
                else{
                    partitions[i]=NUM_WAY/NUM_CPUS+1;
                }
                cout<<"for cpu["<<i<<"] number of ways alloted is/are = "<<partitions[i]<<endl;
            }
            umons_counts = (uint64_t**)malloc(NUM_CPUS*(sizeof(uint64_t*)));
            for (uint32_t i=0; i<NUM_CPUS; i++) {
                umons_counts[i] = (uint64_t*)malloc(LLC_WAY*(sizeof(uint64_t)));
            }
            umon = (cell***)malloc(NUM_CPUS*(sizeof(cell**)));
            for (uint32_t j=0; j<NUM_CPUS; j++) {
            umon[j] = (cell**)malloc(test_sets*(sizeof(cell*)));
            for (uint32_t i=0; i<test_sets; i++) {
                umon[j][i] = (cell*)malloc(LLC_WAY*(sizeof(cell)));
            }
            }
        }
        // cache block
    ...
    void add_mshr(PACKET *packet),
         ...
         lru_updat(uint32_t set, uint32_t way),
         lru_update(uint32_t set, uint32_t way,uint32_t cpu, uint32_t full_addr),
    ...
    uint32_t get_set(uint64_t address),
             ...
             lru_victi(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             lru_victim_llc(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type,uint16_t * temppar),
             umon_lru_victim(uint32_t cpu, uint32_t set, uint64_t full_addr);
    void     atd_replacement(uint32_t cpu, uint32_t set, uint64_t tag);
    void    umon_lru_updat(uint32_t cpu, uint32_t set, uint32_t way);
    void UPDATE_PARTITION(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    void UPDATE_PARTITIONS(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    void get_new_part(int cpus,int ways);
    void print_part();
    void print_umon(int cpu,uint32_t set);
    float getmumax(int cpu, int current, int balance, int* breq);
